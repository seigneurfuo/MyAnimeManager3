...
class PlanningTab(QWidget):
    ...
    def fill_to_watch_table(self) -> None:
        ...
        if self.parent.parent.settings["planning_to_watched_alternative_order"]:
            episodes_to_watch = episodes_to_watch.join(Planning, peewee.JOIN.LEFT_OUTER).group_by(Seasons.id).order_by(peewee.fn.MAX(Planning.date).desc(), Planning.id.desc())
